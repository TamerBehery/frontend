...
CREATE TABLE [dbo].[Login_Users] (
    ...
    [image] NVARCHAR(1000),
    [createdAt] DATETIME2 NOT NULL CONSTRAINT [Login_Users_createdAt_df] DEFAULT CURRENT_TIMESTAMP,
    [updatedAt] DATETIME2 NOT NULL,
    CONSTRAINT [Login_Users_pkey] PRIMARY KEY CLUSTERED ([id]),
    CONSTRAINT [Login_Users_username_key] UNIQUE NONCLUSTERED ([username]),
    CONSTRAINT [Login_Users_email_key] UNIQUE NONCLUSTERED ([email])
);
...
-- AddForeignKey
ALTER TABLE [dbo].[Account] ADD CONSTRAINT [Account_userId_fkey] FOREIGN KEY ([userId]) REFERENCES [dbo].[Login_Users]([id]) ON DELETE NO ACTION ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE [dbo].[Session] ADD CONSTRAINT [Session_userId_fkey] FOREIGN KEY ([userId]) REFERENCES [dbo].[Login_Users]([id]) ON DELETE NO ACTION ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE [dbo].[Authenticator] ADD CONSTRAINT [Authenticator_userId_fkey] FOREIGN KEY ([userId]) REFERENCES [dbo].[Login_Users]([id]) ON DELETE CASCADE ON UPDATE CASCADE;
